...
const run = async () => {
  ...
  const workerArrayBufferTime = await calcTime(
    primeRange,
    arrayBuffer,
    "worker arrayBuffer"
  );
  const workerSharedArrayBufferTime = await calcTime(
    primeRange,
    sharedArrayBuffer,
  ...
  console.log(
    `⏱️ Time difference between main thread and worker (sharedArrayBuffer) : ${(localTime -
      workerSharedArrayBufferTime) /
      NS_PER_SEC} seconds`
  );
};
